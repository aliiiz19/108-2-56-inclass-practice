...
length(newTaipeiCityVaccine) # 可選位置
names(newTaipeiCityVaccine) # 可選名字
```
...
```{r}
str(tainanSportsMap, max.leve = 1)
str(tainanSportsMap[c(1, 563, 1011)], max.level = 1)
```
...
str(taipeiNightMarkets[c(6,7)], max.level=1)
str(taipeiNightMarkets[c("南機場夜市","景美夜市")], max.level=1)
```
